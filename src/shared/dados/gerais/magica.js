...
export async function Magica(connection) {
  for (const key in magicPowers) {
    const power = magicPowers[key];
    ...
    try {
      const [rows] = await connection.execute(
        `INSERT INTO magic_powers (name, description, type, requirements) VALUES (?, ?, ?, ?)`,
