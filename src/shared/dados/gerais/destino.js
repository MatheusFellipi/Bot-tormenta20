export const DestinyPowers = {
  ...
  APARENCIA_INOFENSIVA: {
    ...
    type: "DESTINO",
    requirements: [
      [{ type: "ATRIBUTO", name: 'Carisma', value: 1 }],
    ],
  },
  ATLETICO: {
    name: 'Atlético',
    ...
    type: "DESTINO",
    requirements: [
      [{ type: "ATRIBUTO", name: 'Força', value: 15 }],
    ],
  },
  ATRAENTE: {
    name: 'Atraente',
  ...
  VENEFICIO: {
    ...
    type: "DESTINO",
    requirements: [
      [{ type: "PERICIA", name: "OFICIO ALQUIMIA" }],
    ],
  },
  VONTADE_DE_FERRO: {
...
export async function Destino(connection) {
  for (const key in DestinyPowers) {
    const power = DestinyPowers[key];
    ...
    try {
      const [rows] = await connection.execute(
        `INSERT INTO destiny_powers (name, description, type, requirements) VALUES (?, ?, ?, ?)`,
