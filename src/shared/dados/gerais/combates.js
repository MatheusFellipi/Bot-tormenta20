...
export async function Combate(connection) {
  for (const key in combatPowers) {
    const power = combatPowers[key];
    ...
    try {
      const [rows] = await connection.execute(
        `INSERT INTO combat_powers (name, description, type, requirements) VALUES (?, ?, ?, ?)`,
